...
此方式适用于服务器已有 Nginx 容器和共享反向代理网络的情况。OpenTV 使用独立数据库与数据卷；仅应用容器加入代理网络，Go、PostgreSQL 和 Redis 留在项目内部网络，所有服务均不发布宿主机端口。
...
```sh
git clone --branch codex/go-worker --single-branch https://github.com/puremixai/OpenTV.git opentv
cd opentv
```
...
```dotenv
OPENTV_PROXY_NETWORK=my_net
OPENTV_IMAGE=opentv:RELEASE_COMMIT
OPENTV_GO_IMAGE=opentv-go-worker:RELEASE_COMMIT
PUID=1001
PGID=1001
```
...
docker compose --env-file .env.go-worker.local -f compose.local.yaml -f compose.go-worker.yaml -f compose.server.yaml build opentv opentv-go
...
服务项目名为 `opentv-server`，数据卷均以 `opentv-server_` 开头。复用此配置部署第二个实例时，需同时修改项目名、卷名和代理网络别名，避免共享数据或错误路由。

根据 [Nginx 模板](../deploy/nginx/opentv.conf.example) 配置域名和证书路径，放入现有 Nginx 的配置目录。模板支持 WebSocket、SSE 和应用容器更新后的 Docker DNS 重新解析。先确认应用与 Go 健康，再执行 `nginx -t` 和无中断 reload。
