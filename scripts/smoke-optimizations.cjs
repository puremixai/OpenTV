...
const name = 'moontvplus-optimization-qa-' + randomBytes(4).toString('hex');
...
(async () => {
  try {
    docker(['run','-d','--name',name,'--init','-p','127.0.0.1::3000','-e','PASSWORD','-e','AUTH_SECRET','-e','CRON_SECRET','-e',`USERNAME=${username}`,'-e',`ADMIN_USERNAME=${username}`,'-e','NEXT_PUBLIC_STORAGE_TYPE=d1','-e','SQLITE_DB_PATH=/tmp/optimization.db','-e','HOSTNAME=0.0.0.0','-e','PORT=3000',process.env.SMOKE_IMAGE || 'moontvplus:optimization-candidate'], { env: { ...process.env, PASSWORD: password, AUTH_SECRET: randomBytes(32).toString('hex'), CRON_SECRET: randomBytes(32).toString('hex') } });
