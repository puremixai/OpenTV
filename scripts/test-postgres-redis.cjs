...
const postgres = `xtv-pg-test-${suffix}`;
const redis = `xtv-redis-test-${suffix}`;
...
(async () => {
  try {
    ...
    for (let i = 0; i < 60; i++) {
      try {
        docker(['exec', postgres, 'pg_isready', '-U', 'test', '-d', 'xtvtest']);
        ready = true;
        break;
      } catch {
        await sleep(500);
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          POSTGRES_URL: `postgresql://test:${password}@${pgAddress}/xtvtest`,
          PG_TEST_URL: `postgresql://test:${password}@${pgAddress}/xtvtest`,
