...
func TestOperations(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"pagecount":"2","total":"3","list":[{"vod_id":123,"vod_name":"Video","vod_play_from":"m3u8","vod_play_url":"A$http://a/1.m3u8#B$http://a/2.mp4$$$C$http://a/3.m3u8#D$http://a/4.m3u8","opentv_episodes":"untrusted"}]}`))
	}))
	defer server.Close()
	h := New(server.Client())
