...
func TestLoadRequiresTokenAndValidConfiguration(t *testing.T) {
	...
	for _, values := range []map[string]string{
		{"OPENTV_GO_TOKEN": "short"},
		{"OPENTV_GO_TOKEN": strings.Repeat("s", 32), "OPENTV_GO_MAX_DOWNLOADS": "0"},
		{"OPENTV_GO_TOKEN": strings.Repeat("s", 32), "OPENTV_GO_ALLOWED_ORIGINS": "not-json"},
		{"OPENTV_GO_TOKEN": strings.Repeat("s", 32), "OPENTV_GO_LISTEN_ADDR": "not-a-listener"},
	} {
		if _, err := Load(get(values)); err == nil {
			t.Fatal("accepted invalid config")
		}
	}
	cfg, err := Load(get(map[string]string{"OPENTV_GO_TOKEN": strings.Repeat("s", 32)}))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadUsesExplicitLegacyDownloadDirectory(t *testing.T) {
	...
	cfg, err := Load(func(key string) string {
		return map[string]string{
			"OPENTV_GO_TOKEN": strings.Repeat("s", 32), "OFFLINE_DOWNLOAD_DIR": root, "OFFLINE_DOWNLOAD_PROXY": "http://proxy.example:8080",
			"OPENTV_GO_ALLOWED_ORIGINS": `["http://openlist:5244"]`, "OPENTV_GO_MAX_DOWNLOADS": "3",
		}[key]
	})
	if err != nil {
		t.Fatal(err)
...
func TestDownloadsRequireExplicitEnable(t *testing.T) {
	for _, value := range []string{"", "false", "TRUE", "true"} {
		cfg, err := Load(func(key string) string {
			return map[string]string{"OPENTV_GO_TOKEN": strings.Repeat("s", 32), "OPENTV_GO_OFFLINE_DOWNLOADS": value}[key]
		})
