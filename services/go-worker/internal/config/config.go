...
func Load(getenv func(string) string) (Config, error) {
	...
	if value := getenv("OPENTV_GO_LISTEN_ADDR"); value != "" {
		cfg.ListenAddr = value
	}
	_, port, err := net.SplitHostPort(cfg.ListenAddr)
	if err != nil {
		return Config{}, errors.New("OPENTV_GO_LISTEN_ADDR must be host:port")
	}
	...
	if value := getenv("OPENTV_GO_ALLOWED_ORIGINS"); value != "" {
		if err = json.Unmarshal([]byte(value), &cfg.AllowedOrigins); err != nil {
			return Config{}, errors.New("OPENTV_GO_ALLOWED_ORIGINS must be a JSON string array")
		}
	}
	cfg.MaxDownloads, err = positive(getenv("OPENTV_GO_MAX_DOWNLOADS"), 2)
	if err != nil {
		return Config{}, err
	}
	cfg.SegmentConcurrency, err = positive(getenv("OPENTV_GO_SEGMENT_CONCURRENCY"), 6)
	if err != nil {
		return Config{}, err
	}
	cfg.ScanConcurrency, err = positive(getenv("OPENTV_GO_SCAN_CONCURRENCY"), 4)
	if err != nil {
		return Config{}, err
	}
	return cfg, nil
