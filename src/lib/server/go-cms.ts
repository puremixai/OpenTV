...
export async function fetchCmsResponse(
  ...
  if (!isGoWorkerEnabled('search')) {
    ...
    response.headers.delete('x-opentv-cms-native');
    return response;
  }
      ...
      if (
        ...
        !document.list.every(
          (item: Record<string, unknown>) =>
            item &&
            Array.isArray(item.opentv_episodes) &&
            item.opentv_episodes.every(
              (url: unknown) => typeof url === 'string',
            ) &&
            Array.isArray(item.opentv_episode_titles) &&
            item.opentv_episode_titles.every(
              (title: unknown) => typeof title === 'string',
            ) &&
            item.opentv_episodes.length === item.opentv_episode_titles.length,
        )
      ) {
  ...
  return new Response([204, 205, 304].includes(response.status) ? null : body, {
    ...
    headers: { 'Content-Type': 'application/json', 'x-opentv-cms-native': '1' },
  });
}
