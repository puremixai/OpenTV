...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.debug('解析到推荐数:', recommendations.length);
    ...
    for (const rec of recommendations) {
      ...
      if (rec.title.includes('...')) {
        logger.debug(`检测到截断标题: ${rec.title}, ID: ${rec.doubanId}`);
        ...
        try {
          ...
          if (detailData && detailData.title) {
            logger.debug(`成功获取完整标题: ${detailData.title}`);
            processedRecommendations.push({
              ...rec,
              title: detailData.title,
            });
          } else {
            logger.debug(`详情接口未返回标题，移除该视频: ${rec.doubanId}`);
            // 补充失败，不添加到结果中
          }
        } catch (error) {
    ...
    logger.debug('处理后的推荐数:', processedRecommendations.length);
