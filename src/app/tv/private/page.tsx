...
export default function TVPrivatePage() {
  ...
  return (
    <TVLayout>
      <section className='rounded-[42px] border border-white/10 bg-slate-950/70 p-8 shadow-2xl shadow-black/60'>
        <div className='flex items-center justify-between gap-6'>
          ...
          <div className='flex gap-3'>
            {enabledSources.map((item) => <button key={item} onClick={() => setSource(item)} className={`cursor-pointer rounded-2xl px-6 py-4 text-2xl font-bold outline-hidden transition tv-focusable ${source === item ? 'bg-rose-600 text-white' : 'bg-white/8 text-slate-200 hover:bg-white/12'}`}>{sourceLabel[item]}</button>)}
          </div>
        </div>
        {source === 'emby' && embySources.length > 1 && <div className='mt-6 flex gap-3 overflow-x-auto px-3 py-3 scrollbar-none'>{embySources.map((item) => <button key={item.key} onClick={() => setEmbyKey(item.key)} className={`cursor-pointer rounded-2xl px-5 py-3 text-xl font-bold ${embyKey === item.key ? 'bg-white text-black' : 'bg-white/10 text-white'}`}><Server className='mr-2 inline h-5 w-5' />{item.name}</button>)}</div>}
      </section>
      ...
      {!loading && enabledSources.length === 0 && <div className='mt-8 rounded-3xl border border-white/10 bg-white/4 p-8 text-2xl text-slate-300'>未启用私人影库源。</div>}
      ...
      {!loading && source === 'xiaoya' && <section className='mt-10 space-y-8'>
        <div className='rounded-3xl bg-white/4 p-5 text-xl text-slate-300'>当前位置：{xiaoyaPath}</div>
        {xiaoyaPath !== '/' && <button onClick={() => setXiaoyaPath('/' + xiaoyaPath.split('/').filter(Boolean).slice(0, -1).join('/'))} className='cursor-pointer rounded-2xl bg-white/10 px-6 py-4 text-2xl font-bold outline-hidden tv-focusable'>返回上级</button>}
        {folders.length > 0 && <div><h2 className='mb-5 text-4xl font-black'>文件夹</h2><div className='grid grid-cols-2 gap-4 md:grid-cols-4'>{folders.map((folder) => <button key={folder.path} onClick={() => setXiaoyaPath(folder.path)} className='flex min-h-24 cursor-pointer items-center gap-3 rounded-3xl border border-white/10 bg-white/6 p-5 text-left text-2xl font-bold outline-hidden tv-focusable'><Folder className='h-9 w-9 text-rose-400' />{folder.name}</button>)}</div></div>}
        {files.length > 0 && <div><h2 className='mb-5 text-4xl font-black'>视频文件</h2><div className='grid gap-4'>{files.map((file) => {
          ...
          return <button key={file.path} onClick={() => router.push(`/tv/play?source=xiaoya&id=${encodeURIComponent(encodedDirPath)}&fileName=${encodeURIComponent(file.name)}&title=${encodeURIComponent(title)}`)} className='flex cursor-pointer items-center gap-4 rounded-3xl border border-white/10 bg-white/6 p-5 text-left text-2xl font-bold outline-hidden tv-focusable'><PlayCircle className='h-9 w-9 text-rose-400' />{file.name}</button>;
        })}</div></div>}
      </section>}
    </TVLayout>
  );
