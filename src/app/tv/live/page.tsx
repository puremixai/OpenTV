  ...
  return (
    <TVLayout>
      ...
      <section className='rounded-[42px] border border-white/10 bg-slate-950/70 p-10 shadow-2xl shadow-black/60'>
        ...
        <div className='mt-8 flex gap-4 overflow-x-auto px-4 py-4 scrollbar-none'>
          {sources.map((item) => (
            <button key={item.key} onClick={() => setSource(item.key)} className={`cursor-pointer rounded-2xl px-6 py-4 text-2xl font-bold outline-hidden transition tv-focusable ${source === item.key ? 'bg-rose-600 text-white' : 'bg-white/8 text-slate-200 hover:bg-white/12'}`}>{item.name}</button>
          ))}
        </div>
        <label className='mt-5 flex h-20 items-center gap-4 rounded-3xl border border-white/10 bg-white/10 px-6 focus-within:border-rose-500'>
          ...
          <input
            ...
            placeholder='搜索频道或分类'
            className='tv-focusable h-16 flex-1 bg-transparent text-2xl font-bold text-white outline-hidden placeholder:text-slate-500'
          />
        </label>
      </section>

      {quickChannels.length > 0 && (
        <section className='mt-8 rounded-[34px] border border-white/10 bg-white/4 p-6'>
      ...
      {error ? (
        <section role='alert' className='mt-10 rounded-[34px] border border-red-500/40 bg-red-950/45 p-8 text-red-100'>
          ...
          <button onClick={() => window.location.reload()} className='tv-focusable mt-6 cursor-pointer rounded-2xl bg-rose-600 px-7 py-4 text-2xl font-black text-white outline-hidden focus:ring-4 focus:ring-rose-300'>重试</button>
        </section>
        ...
        <div className='mt-10 grid grid-cols-[280px_1fr] gap-6'>
          <aside className='rounded-[32px] border border-white/10 bg-white/4 p-4'>
            <div className='max-h-[70vh] space-y-2 overflow-y-auto pr-2'>
              {groups.map((group) => <button key={group} onClick={() => { setSelectedGroup(group); setVisibleCount(120); }} className={`tv-focusable w-full cursor-pointer rounded-2xl px-5 py-4 text-left text-2xl font-bold outline-hidden focus:ring-4 focus:ring-rose-300 ${selectedGroup === group ? 'bg-rose-600 text-white' : 'text-slate-200 hover:bg-white/10'}`}>{group}</button>)}
            </div>
          </aside>
          <section>
            ...
            {visibleCount < filteredChannels.length && (
              <button onClick={() => setVisibleCount((v) => v + 120)} className='tv-focusable mx-auto mt-8 block cursor-pointer rounded-3xl bg-white/10 px-10 py-5 text-2xl font-black text-white outline-hidden focus:ring-4 focus:ring-rose-300'>
                加载更多频道
              </button>
            )}
          </section>
