...
export default function TVCard({ item }: { item: TVItem }) {
  ...
  return (
    <button
      ...
      className='group w-[210px] shrink-0 cursor-pointer rounded-[28px] bg-white/4 p-3 text-left outline-hidden transition duration-200 hover:bg-white/10 focus-visible:bg-white/10 tv-focusable'
    >
      <div className='relative aspect-2/3 overflow-hidden rounded-[22px] bg-slate-900 shadow-xl shadow-black/50 transition duration-200 group-hover:scale-[1.03] group-focus-visible:scale-[1.03]'>
        ...
        <div className='absolute inset-x-0 bottom-0 bg-linear-to-t from-black/90 to-transparent p-3'>
          <div className='inline-flex items-center gap-1 rounded-full bg-black/60 px-2 py-1 text-sm text-amber-300 backdrop-blur-sm'>
