...
// Opt-in: OPENTV_GO_TEST_BINARY points to a locally built worker. No user services/data are used.
require('./web-globals');
...
const binary = process.env.OPENTV_GO_TEST_BINARY;
...
suite('real Node → Go → fixture compatibility', () => {
  ...
  async function startWorker(downloads, directory) {
    const child = spawn(path.resolve(binary), [], {
      ...
      env: {
        ...process.env,
        OPENTV_GO_TOKEN: token,
        OPENTV_GO_LISTEN_ADDR: '127.0.0.1:0',
        OPENTV_GO_OFFLINE_DOWNLOADS: String(downloads),
        OFFLINE_DOWNLOAD_DIR: directory,
        OPENTV_GO_ALLOWED_ORIGINS: JSON.stringify([origin]),
  ...
  beforeAll(async () => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'opentv-go-integration-'));
    ...
    jest.replaceProperty(process, 'env', {
      ...
      OPENTV_GO_URL: workerURL,
      OPENTV_GO_TOKEN: token,
      OPENTV_GO_OFFLINE_DOWNLOADS: 'true',
    });
    jest.resetModules();
  ...
  const request = (method, suffix = '', body) =>
    new NextRequest(`http://opentv.test/api/offline-download${suffix}`, {
      method,
      ...(body
        ? {
