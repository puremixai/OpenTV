...
beforeEach(() => {
  jest.resetModules();
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'opentv-go-bridge-'));
  jest.replaceProperty(process, 'env', {
    NODE_ENV: 'test',
    NEXT_PUBLIC_ENABLE_OFFLINE_DOWNLOAD: 'true',
    OFFLINE_DOWNLOAD_DIR: root,
    OPENTV_GO_URL: 'http://worker:8081',
    OPENTV_GO_TOKEN: 's'.repeat(32),
...
const request = (method, suffix = '', body) =>
  new NextRequest(`https://opentv.example/api/offline-download${suffix}`, {
    method,
    headers: {
...
test('default mode keeps the existing Node implementation without contacting Go', async () => {
  delete process.env.OPENTV_GO_OFFLINE_DOWNLOADS;
...
test('streaming request size is bounded before contacting Go', async () => {
  ...
  const response = await routes.POST(
    new NextRequest('http://opentv.test/api/offline-download', {
      method: 'POST',
      body,
      duplex: 'half',
    }),
