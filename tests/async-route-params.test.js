...
beforeEach(() => {
  ...
  process.env.SITE_BASE = 'https://moontv.test';
...
const movieRequest = (method, body) =>
  new NextRequest('https://moontv.test/api/movie-requests/movie-42', {
    method,
    ...(body
      ? {
...
test('preserves token authorization and the token in an OpenList proxy redirect', async () => {
  ...
  const response = await openListPlay(
    new NextRequest(
      'https://moontv.test/api/openlist/play/media-token?folder=movies&fileName=clip.mp4'
    ),
    { params: Promise.resolve({ token: 'media-token' }) }
  );
  expect(response.status).toBe(307);
  expect(response.headers.get('location')).toBe(
    'https://moontv.test/api/openlist/proxy/media-token/video.mp4?folder=movies&fileName=clip.mp4'
  );
});
...
test('streams subtitles using both the asynchronous token and filename format', async () => {
  ...
  const response = await embySubtitle(
    new NextRequest(
      'https://moontv.test/api/emby/subtitle/media-token/subtitle.ass?itemId=item-1&mediaSourceId=source-1&streamIndex=2'
    ),
    {
...
test('rejects an invalid asynchronous media token without fetching a subtitle', async () => {
  ...
  const response = await embySubtitle(
    new NextRequest(
      'https://moontv.test/api/emby/subtitle/invalid/subtitle.ass?itemId=item-1&mediaSourceId=source-1&streamIndex=2'
    ),
...
test('fetches the Bangumi subject selected by an asynchronous numeric ID', async () => {
  ...
  const response = await bangumiSubject(
    new NextRequest('https://moontv.test/api/bangumi/v0/subjects/123'),
    {
...
test('rejects a malformed asynchronous Bangumi ID before calling the upstream', async () => {
  const response = await bangumiSubject(
    new NextRequest('https://moontv.test/api/bangumi/v0/subjects/not-an-id'),
    {
