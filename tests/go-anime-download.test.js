...
beforeEach(()=>{jest.clearAllMocks();jest.replaceProperty(process,'env',{NODE_ENV:'test',OPENTV_GO_ANIME_DOWNLOADS:'true',OPENTV_GO_URL:'http://worker:8081',OPENTV_GO_TOKEN:'s'.repeat(32)});global.fetch=jest.fn(async()=>Response.json({code:200,replayed:false}));OpenListClient.mockImplementation(()=>({getToken:jest.fn(async()=>'node-token')}));});
...
test('flag off preserves direct Node submission',async()=>{process.env.OPENTV_GO_ANIME_DOWNLOADS='false';await addOpenListOfflineDownload(config,'/anime','magnet:x','aria2',{owner:'alice',key:'ep'});expect(OpenListClient).toHaveBeenCalled();expect(global.fetch.mock.calls[0][0]).toBe('https://openlist.example/api/fs/add_offline_download');});
