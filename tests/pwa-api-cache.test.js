...
let fixture;
beforeAll(async () => {
  fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'xtv-pwa-test-'));
  for (const dir of [
    'public/icons',
    'public/fonts',
    'public/players',
...
test('activation removes old private entries but retains static and offline video assets', async () => {
  ...
  const paths = [
    ...
    '/admin',
    '/_next/static/a.js',
    '/__moontv_idb_video__/movie/playlist.m3u8',
  ];
...
test('generated worker imports the push and IndexedDB offline video handlers', async () => {
  ...
  expect(
    await (
      await worker.request('/__moontv_idb_video__/movie/playlist.m3u8')
    ).text()
  ).toBe('#EXTM3U\nsegment_00000.ts');
  expect(
    await (
      await worker.request('/__moontv_idb_video__/movie/segment_00000.ts')
    ).text()
  ).toBe('video-segment');
...
test('a custom scope generates matching static and push URLs without caching scoped APIs', async () => {
  execFileSync(
    ...
    [path.join(__dirname, '../scripts/generate-pwa.cjs'), '--root', fixture],
    {
      ...
      env: {
        ...
        CF_PAGES: '',
        EDGEONE_PAGES: '',
        NEXT_PUBLIC_BASE_PATH: '/xtv/',
      },
    }
  );
  const worker = createWorkerRuntime(path.join(fixture, 'public'), '/xtv/');
  await worker.install();
  await worker.activate();
  expect(worker.handlers.has('push')).toBe(true);
  const response = await worker.request('/xtv/api/favorites');
  expect(response.status).toBe(200);
  expect(
  ...
  worker.setOffline(true);
  expect(
    await (await worker.request('/xtv/_next/static/chunks/app.js')).text()
  ).toBe('network:/xtv/_next/static/chunks/app.js');
});
