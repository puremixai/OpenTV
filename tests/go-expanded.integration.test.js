...
suite('expanded worker with real isolated upstream and state', () => {
  ...
  async function boot() {
    child = spawn(path.resolve(binary), [], {
      ...
      env: {
        ...
        OPENTV_GO_LOCAL_FILES: 'true',
        OPENTV_GO_TASKS: 'true',
        OPENTV_GO_NETDISK_CHECK: 'true',
        OPENTV_GO_ANIME_DOWNLOADS: 'true',
        OPENTV_GO_STATE_DIR: path.join(root, 'state'),
    ...
    process.env.OPENTV_GO_URL = workerURL;
  }
  async function stop() {
  ...
  beforeAll(async () => {
    ...
    process.env = {
      ...
      OPENTV_GO_TOKEN: token,
      OPENTV_GO_TASKS: 'true',
      OPENTV_GO_SEARCH: 'true',
    };
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'opentv-expanded-'));
