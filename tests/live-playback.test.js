...
test.each([
  ...
  'known direct stream %s needs no OpenTV media request',
  async (url, type) => {
...
test.each(['full', 'm3u8-only'])(
  ...
  async (proxyMode) => {
    ...
    const url = new URL(result.url, 'https://opentv.example');
    expect(url.pathname).toBe('/api/proxy/m3u8');
    expect(url.searchParams.get('url')).toBe(raw);
    expect(url.searchParams.get('moontv-source')).toBe('news & sports');
...
test('an extensionless source gets only a type probe and still plays directly', async () => {
  ...
  expect(request.searchParams.get('moontv-source')).toBe('news & sports');
});
...
test('playlist-only mode preserves direct media, keys and init segments through child playlists', () => {
  ...
  const body = rewriteMediaPlaylist(
    ...
    {
      origin: entry.origin,
      source: entry.searchParams.get('moontv-source'),
      mode: 'live',
