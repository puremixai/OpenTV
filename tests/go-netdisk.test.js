...
beforeEach(() => { jest.clearAllMocks(); jest.replaceProperty(process, 'env', { NODE_ENV:'test', OPENTV_GO_NETDISK_CHECK:'true', OPENTV_GO_URL:'http://worker:8081', OPENTV_GO_TOKEN:'s'.repeat(32) }); requireFeaturePermission.mockResolvedValue({username:'alice'}); global.fetch=jest.fn(async()=>Response.json({task:{id:'abc'}})); });
...
test('disabled retains original Node behavior',async()=>{process.env.OPENTV_GO_NETDISK_CHECK='false';fallback.startNetdiskCheckTask.mockReturnValue({id:'node'});const res=await start(req('start',{platform:'aliyun',links:['x']}));expect((await res.json()).taskId).toBe('node');expect(global.fetch).not.toHaveBeenCalled();});
